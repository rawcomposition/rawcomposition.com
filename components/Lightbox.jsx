  ...
  return (
    <div className="fixed w-full h-full left-0 top-0 flex items-center justify-center p-3 bg-black/80 text-white">
      {imageId && !isLoading ? (
        <figure>
          <img
            src={`https://cdn.download.ams.birds.cornell.edu/api/v1/asset/${imageId}/2400`}
            className="h-full max-h-[calc(100vh-90px)] max-w-full"
          />
      ...
      <button className="absolute top-2 right-6 outline-none text-[2.5rem] font-bold" onClick={handleClose}>
        &times;
      </button>
    </div>
  );
